...
export default function OnboardingPage() {
  ...
  async function handleFinish() {
    ...
    // Re-fetch session at click time as fallback
    ...
    <>
      ...
      <div style={{ minHeight:"100vh", display:"flex", alignItems:"center", justifyContent:"center", padding:24, background:"#0a0a0f" }}>
        <div style={{ width:"100%", maxWidth:560, animation:"fadeUp 0.4s ease both" }}>
            ...
            <div style={{ display:"flex", justifyContent:"center", gap:8 }}>
              {[1,2,3].map((s) => (
                <div key={s} style={{
                  ...
                  background:s===step?"#f5a623":s<step?"rgba(245,166,35,0.5)":"rgba(255,255,255,0.1)",
                  transition:"all 0.3s"
                }} />
              ))}
            </div>
          ...
          {step === 1 && (
            <div>
              <h2 style={{ fontFamily:"'Clash Display',sans-serif", fontSize:22, fontWeight:600, marginBottom:8, textAlign:"center" }}>
                What kind of work do you do?
              </h2>
              <p style={{ fontSize:14, color:"#6b6b80", textAlign:"center", marginBottom:28 }}>
                This determines which tax section applies to you
              </p>
          ...
          {step === 2 && (
            <div>
              ...
              <p style={{ fontSize:14, color:"#6b6b80", textAlign:"center", marginBottom:28 }}>
                Used to match your TDS records. Never shared.
              </p>

              <div style={{ background:"#111118", border:"1px solid rgba(255,255,255,0.08)", borderRadius:16, padding:28 }}>
                <label style={{ display:"block", fontSize:12, color:"#6b6b80", marginBottom:8, textTransform:"uppercase", letterSpacing:"0.8px" }}>
                  PAN Number
                </label>
                <input
                  className="ob-input"
                  ...
                  maxLength={10}
                  style={{ fontSize:18, letterSpacing:4, fontWeight:500, textAlign:"center" }}
                />
                <p style={{ fontSize:12, color:"#6b6b80", marginTop:10, textAlign:"center" }}>
                  Format: 5 letters + 4 digits + 1 letter
                </p>
              </div>

              <div style={{ display:"flex", gap:10, marginTop:20 }}>
                <button className="ob-btn ob-ghost" onClick={() => setStep(1)} style={{ flex:1, padding:"13px" }}>
                  ← Back
                </button>
          ...
          {step === 3 && (
            <div>
              ...
              <p style={{ fontSize:14, color:"#6b6b80", textAlign:"center", marginBottom:28 }}>
                Don't worry — we'll calculate both and recommend the better one
              </p>

              <div style={{ display:"flex", flexDirection:"column", gap:12, marginBottom:20 }}>
                {[
                  { value:"new" as const, label:"New Regime (Default)", desc:"Lower tax rates, fewer deductions. Best for most freelancers.", recommended:true },
                  { value:"old" as const, label:"Old Regime", desc:"Higher rates but allows 80C, HRA, and other deductions.", recommended:false },
                ].map((r) => (
                  <button
                    key={r.value}
                  ...
                  >
                    <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:6 }}>
                      ...
                      {r.recommended && (
                        <span style={{ fontSize:10, background:"rgba(245,166,35,0.15)", color:"#f5a623", padding:"2px 8px", borderRadius:4, fontWeight:600 }}>
                          RECOMMENDED
                        </span>
                      )}
                    </div>
              ...
              <div style={{ display:"flex", gap:10 }}>
                <button className="ob-btn ob-ghost" onClick={() => setStep(2)} style={{ flex:1, padding:"13px" }}>
                  ← Back
                </button>
